...
using DictionaryView = std::vector<const std::string*>;

inline DictionaryView dict_view(const Dictionary& dict)
{
	DictionaryView view(dict.size());
...
using Word = DictionaryView::value_type;
...
public: // Functions
	...
	Results make_guess(const Word guess);
